...
\section{Đánh giá phần cứng}
\subsection {Mặt tích cực}
...
Trong quá trình thiết kế board, nhóm có thực hiện sai việc kết nối 1 dây ở vị trí mạch chuyển đổi áp sang dòng ở hình \ref{fig:wireerror}, nhóm đã thực hiện bằng cách cắt đường mạch và nối dây bên ngoài, hình \ref{fig:powererror}.Bên cạnh đó, trong quá trình lựa chọn opamp để mô phỏng và sử dụng, nhóm dùng OPAMP INA333, khi mô phỏng để đạt được hiệu quả mong muốn nhóm đã cấp nguồn cho opamp là 12V và -5V, kết quả mô phỏng diễn ra suôn sẻ đã được thể hiện ở chương trước, trong quá trình thiết kế phần cứng thực tế nhóm đã sơ suất trong việc xem xét nguồn cấp cho INA333 dẫn đến kết quả đo đạc không đúng, nhóm đã có những điều chỉnh kết nối để INA333 hoạt động bình thường, hình \ref{fig:afrerfix} sau khi đã sửa. Trong quá trình thiết kế mô phỏng và thiết kế phần cứng nhóm có thiết kế một mạch đo và bù nhiệt độ sai số do loadcell, nhưng trong quá trình thực hiện phần cứng nhóm có một số lỗi như đã trình bày nên khi đo đạc thực tế nhóm chưa sử dụng chức năng bù nhiệt đó, một phần là đang đo thử ở 1 nhiệt độ cố định và một phần là để đảm bảo kết quả ổn định, phần bù nhiệt nhóm sẽ cố gắng ứng dụng trong tương lai để tối ưu hóa được độ chính xác đo đạc.


\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{image/wireerror.jpg}
    \caption{Lỗi kết nối dây}
    \label{fig:wireerror}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{image/powererror.jpg}
    \caption{Lỗi chọn nguồn}
    \label{fig:powererror}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{image/afterfix.jpg}
    \caption{Sau khi chỉnh sửa phần cứng}
    \label{fig:afrerfix}
\end{figure}